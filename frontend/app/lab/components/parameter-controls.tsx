...
export default function ParameterControls(props: Props) {
  const { temperature, topP, topK, maxOutputTokens, onChange, disabled } =
    props;
  return (
    <div className={cn("grid gap-4")}>
      <Row
        label="Temperature"
        tooltip="Controls randomness; higher = more diverse"
      ...
      <Row
        label="Top P"
        tooltip="Nucleus sampling; probability mass to sample from"
        value={Number(topP.toFixed(2))}
      >
        <Slider
          value={[topP]}
          min={RANGES.topP.min}
      ...
      <Row label="Top K" tooltip="Sample from top K tokens" value={topK}>
        <Slider
          value={[topK]}
          min={RANGES.topK.min}
          max={RANGES.topK.max}
      ...
      <Row
        ...
        tooltip="Upper bound on generated tokens"
